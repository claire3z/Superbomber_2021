...
BOMB_NULL = 'BOMBING_WITHOUT_DESTROY'
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state is not None:
        ...
        # discourage dropping bomb without targets
        if e.BOMB_EXPLODED and not (e.KILLED_OPPONENT or e.CRATE_DESTROYED):
            events.append(BOMB_NULL)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    filename = f"{path}Q_{round+20000}"
    if round in [i for i in range(1000,31000,1000)]:
        with open(filename+'_eps_0.5.pk', "wb") as file:
            pickle.dump(self.Q, file)

        with open(filename+"_eps_0.5_tracker.pk", "wb") as file:
            pickle.dump(self.Q_tracker, file)
...
def reward_from_events(self, events: List[str]) -> int:
    ...
    game_rewards_b4 = {
        ...
        # incentives
        # e.BOMB_EXPLODED: 0.1, Should not incentivise other wise bombing without purpose
        e.CRATE_DESTROYED: 0.1,
        e.COIN_FOUND: 0.1,
        e.KILLED_OPPONENT: 5,
        # e.SURVIVED_ROUND: 5,
        BOMB_NULL: -0.5 # to discourage non target bombing
    }

    game_rewards_p4 = {
        ESCAPED: 0.125, #reward for each step in the path of last 4 steps away from bombing
        e.KILLED_SELF: -0.125,
    }
