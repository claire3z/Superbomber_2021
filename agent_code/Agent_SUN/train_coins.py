...
import events as e
from .callbacks import infer_local_features, state_to_features,features_to_stateIndex, ACTIONS, Q_save, eps_train
...
Q_tracker = 'model/Q_tracker_SUN.pk' # initialize once and keep tracking of update stats in Q-table
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    filename = f"model/coins/Q_SUN_{round}_{self.alpha}_{self.gamma}_{self.eps_train}"
    if round % 1000 == 0:
        with open(filename, "wb") as file:
            pickle.dump(self.Q, file)

        with open(filename+"_tracker", "wb") as file:
            pickle.dump(self.Q_tracker, file)
